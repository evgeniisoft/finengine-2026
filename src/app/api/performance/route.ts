...
async function gasGet(sheet: string): Promise<{ data: any[]; elapsed_ms: number }> {
...
export async function GET(request: NextRequest) {
    try {
        ...
        const sheets = ['Companies', 'Accounts', 'Transactions', 'Settings', 'Budgets', 'Counterparties'];
        ...
        for (const sheet of sheets) {
            ...
            results.push({
                ...
                rows: data.length,
                time_ms: elapsed_ms,
                status: elapsed_ms > 3000 ? 'critical' : elapsed_ms > 1500 ? 'warning' : 'ok'
            });
        }
        ...
        results.push({
            ...
            status: parallelElapsed > 5000 ? 'critical' : parallelElapsed > 2500 ? 'warning' : 'ok'
        });
        ...
        for (const company of compData) {
            calculator.calculatePnL(txData, accData, company.id, '2026-01-01', '2026-12-31', company);
        }
        ...
        for (const company of compData) {
            calculator.calculateCashFlow(txData, accData, company.id, '2026-01-01', '2026-12-31', company);
        }
        ...
        for (const company of compData) {
            calculator.calculateBalanceSheet(txData, accData, company.id, '2026-12-31', company);
        }
            ...
            summary: {
                ...
                recommendations: [
                    slowest.time_ms > 3000
                        ? `${slowest.name} — узкое место (${slowest.time_ms}мс). Рассмотрите оптимизацию.`
