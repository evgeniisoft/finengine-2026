      ...
      <div className="flex items-center gap-4 mb-4">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
            viewMode === 'consolidated' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50'
          }`}
        >
          Консолидированный
        </button>
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
            viewMode === 'by_company' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50'
          }`}
        >
          По компаниям
        </button>
          ...
          <div className="ml-auto flex gap-2">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium ${
                showMonthly ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'
              }`}
            >
      ...
      {rows.map(row => (
        <div key={row.id}>
          ...
          >
            <span className={`text-sm ${row.bold ? 'font-semibold text-gray-900' : 'text-gray-600'}`}>{row.label}</span>
            <span className={`text-sm ${row.bold ? 'font-bold' : 'font-medium'} ${
              row.type === 'inflow' ? 'text-green-600' : row.type === 'outflow' ? 'text-red-600' : 'text-gray-900'
            }`}>
              {row.value?.toLocaleString('ru-RU') || 0} ₽
            </span>
          </div>
...
function CalendarView({ transactions, viewMode, companies }: any) {
  const [days, setDays] = useState(30);
  
  // Фильтруем транзакции по viewMode
  const filteredTx = viewMode === 'by_company' && companies.length > 0
    ? transactions.filter((t: any) => t.company_id === companies[0]?.id)
    : transactions;
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
      <div className="flex justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-900">
          {viewMode === 'consolidated' ? 'Консолидированный платёжный календарь' : 'Платёжный календарь'}
        </h3>
...
function CashGapsView({ transactions, viewMode, companies }: any) {
  const [days, setDays] = useState(30);
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
      <div className="flex justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-900">
          {viewMode === 'consolidated' ? 'Консолидированные кассовые разрывы' : 'Кассовые разрывы'}
        </h3>
        <select value={days} onChange={(e) => setDays(parseInt(e.target.value))}
          className="px-3 py-2 border border-gray-300 rounded-lg text-sm">
          <option value={7}>7 дней</option>
