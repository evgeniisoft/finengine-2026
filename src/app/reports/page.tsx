...
function MonthlyTableView({ data, type, onDrilldown, drilldownData, drilldownLoading, activeDrilldown }: any) {
    ...
    // Периоды — колонки (горизонтально)
    const periods = data.map((d: any) => d.period || d.month).filter(Boolean);

    // Строки — статьи (зависит от типа отчёта)
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            {periods.map((p: string) => (
                                <th key={p} className="px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase whitespace-nowrap">
                                    {p}
                                </th>
                            ))}
                        </tr>
                    ...
                    <tbody className="divide-y divide-gray-200">
                        {rows.map((row: any) => (
                            ...
                            >
                                <td className={`px-4 py-3 text-sm sticky left-0 bg-white ${row.bold ? 'font-semibold text-gray-900' : 'text-gray-600'}`}>
                                    {row.label}
                                </td>
                                {data.map((d: any) => (
                                    <td key={d.period || d.month} className={`px-6 py-3 text-sm text-right whitespace-nowrap ${row.bold ? 'font-bold' : 'font-medium'} ${row.color}`}>
...
function getRowsForType(type: string) {
    switch (type) {
        case 'pnl':
            return [
